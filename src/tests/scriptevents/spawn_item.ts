...
new Test("scriptevent_spawn_item", "empty")
    ...
    .run(async (player) => {
        ...
        const beforeCount = player.dimension.getEntities({ type: "minecraft:item", location: spawnLocation, maxDistance: 3 }).length;

        player.runCommand(`scriptevent capi:${config.scriptevents.spawn_item.name} ${JSON.stringify(itemData)}`);
        await system.waitTicks(5);

        const afterCount = player.dimension.getEntities({ type: "minecraft:item", location: spawnLocation, maxDistance: 3 }).length;
        ...
        // Cleanup
        const items = player.dimension.getEntities({ type: "minecraft:item", location: spawnLocation, maxDistance: 3 });
